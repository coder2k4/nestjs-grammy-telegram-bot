...
export class TelegrambotService implements OnModuleInit {
  ...
  activeChats = new Map<number, number>(); // Хранит связь между пользователями и оператором
  ...
  constructor(private configService: ConfigService) {
    this.ownerId = this.configService.get<string>('TELEGRAM_OWNER_ID');
  ...
  onModuleInit() {
    ...
    this.bot.command('call', async (ctx) => {
      ...
      await ctx.reply(
        'Чтобы мы с Вами могли связаться, нам нужны ваши контактные данные',
        {
    ...
    📊 Рассчитанный объем: ${volume.toFixed(2)} рублей.
  `;

      // Отправляем сообщение менеджеру (замените this.ownerId на ID менеджера)
      await this.bot.api.sendMessage(this.ownerId, messageToManager);

      // Уведомляем пользователя, что данные были отправлены
      await ctx.reply(
        'Разрешите доступ к вашему контакту набрав команду /call (или выбрав иж меню) - иначе у нас не будет возможности связаться с Вами.',
      );
    });
    ...
    //Калькулятор
    {
      ...
      this.bot.callbackQuery(/cardboard_(.+)/, async (ctx) => {
        ...
        await ctx.replyWithPhoto(new InputFile('src/shared/images/height.jpg'));
        await ctx.reply(
    ...
    this.bot.command('online_consultation', async (ctx) => {
      ...
      await ctx.reply(
        'Спасибо за запрос! Оператор скоро свяжется с вами для консультации.',
      );

      // Сохраняем чат
      this.activeChats.set(ctx.from.id, this.ownerId);
    });
    ...
    `;
      ...
      await this.bot.api.sendMessage(this.ownerId, message);
    ...
    this.bot.on('message:text', async (ctx) => {
      const operatorId = this.activeChats.get(ctx.from.id);
      ...
      if (ctx.from.id == this.ownerId && ctx.message.reply_to_message) {
        // Извлекаем ID пользователя из текста ответа оператора
        const userIdMatch =
          ctx.message.reply_to_message.text.match(/\((\d+)\)/);
        if (userIdMatch) {
          const userId = parseInt(userIdMatch[1], 10);

          // Пересылаем сообщение от оператора пользователю
          await this.bot.api.sendMessage(
            userId,
  ...
  calculateVolume(sessionData: any) {
    const typeEff = this.coefficients.type[sessionData.type].efficiency;
    const paperEff = this.coefficients.color[sessionData.color].efficiency;
    const cardboardEff =
    ...
    const baseVolume = (height * width * length) / 1000;

    // Финальный расчет с коэффициентами
    return baseVolume * typeEff * paperEff * cardboardEff;
  }
}
